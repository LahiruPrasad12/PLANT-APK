...
export default {
  ...
  methods: {
    ...
    handleSuccess(position) {
      ...
      //         // Extract and set the location text
      //         locationText.value = response.data.results[0].formatted_address;
      //       } else {
    ...
    async predictImage() {
      try {
        ...
        let respond = (await prediction_api.predictImage(payload)).data.message;
        this.predicted_name = respond.type;
        this.predicted_week = respond.week;
    ...
    async uploadPredictedData() {
      try {
        ...
        let payload = {
          ...
          predicted_week: this.predicted_week,
        };
